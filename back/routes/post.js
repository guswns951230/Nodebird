...
router.post('/:postId/retweet', isLoggedIn, async (req, res, next) => {  // Post /post/1/commnet
  try {
    ...
    if (req.user.id === post.UserId || (post.Retweet && post.Retweet.UserId === req.user.id)) {
      return res.status(403).send('자신의 게시물은 retweet할 수 없습니다.');
    }
    ...
    if (exPost) {
      return res.status(403).send('이미 retweet한 게시글입니다.');
    }
...
router.post('/:postId/comment', isLoggedIn, async (req, res, next) => {  // Post /post/1/commnet
  try {
    const post = await Post.findOne({
      where: { id: req.params.postId },
    });
